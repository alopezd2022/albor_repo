...
def generate_launch_description():
    moveit_config = MoveItConfigsBuilder("albor", package_name="albor_moveit_config").to_moveit_configs()
    ...
    pkg_path = get_package_share_directory('albor_description')
    ...
    model_path = get_model_paths(['albor_description'])

    robot_description_launcher = IncludeLaunchDescription(
       PathJoinSubstitution(
           [FindPackageShare("albor_moveit_config"), "launch", "rsp.launch.py"]
       ),
    )
    ...
    bridge = Node(
        ...
        parameters=[
            {
                'config_file': join(
                    pkg_path, 'config', 'albor_bridge.yaml'
                ),
                'use_sim_time': True,
            }
        ],
    ...
    rviz_config_file = PathJoinSubstitution(
        [FindPackageShare("albor_description"), "rviz", "albor.rviz"]
    )
    rviz_node = Node(
        package="rviz2",
        executable="rviz2",
